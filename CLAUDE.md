...
- **restart-app** — Restart a running app via `restart_app` without closing the lease. Per the `scripts/session-start.sh` runtime policy, `restart_app` is a provider HTTPS call, NOT a Cosmos broadcast — no gas, no fee estimate, no `cosmos_estimate_fee` step. The skill inlines its own textual confirm. PreToolUse still gates the tool. Pre-call: refuse unless `chainState.state === 2 || chainState.state === 'LEASE_STATE_ACTIVE'` (inline enum decode — the canonical Cosmos lease enum is `0=UNSPECIFIED, 1=PENDING, 2=ACTIVE, 3=INSUFFICIENT_FUNDS, 4=CLOSED`, stable per ENG-158; the chain returns either the integer or the canonical string depending on the encoding path, so check both forms). Post-call: re-query `app_status` once and inline-decode the new state with the same dual-form check; tag the journal `recovery_actions` with `["restart-post-verify-not-active"]` on regression.
