...
- **restart-app** — Restart a running app via `restart_app` without closing the lease. Broadcast skill, but `restart_app` is `estimable: false` so the textual confirm is inlined (no `cosmos_estimate_fee` step) rather than threaded through `references/billing-tx-confirm.md`. Pre-broadcast: refuse if `chainState.state` is terminal. Post-broadcast: re-query `app_status` once and surface the new `provision_status`.
