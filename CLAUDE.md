...
- **DECISION 5 — `manage-domain` lookup branches in skill prose.** Set/clear route through `mcp__manifest-agent__manage_domain_orchestrated`; lookup calls `mcp__manifest-lease__lease_by_custom_domain` directly. Rationale: the orchestrated wrapper drives an elicitation flow designed for state-changing actions (fee estimation, intent confirm, on-chain verification), which is unnecessary overhead for a pure chain query. (The wrapper itself is NOT PreToolUse-gated — the matcher in `hooks/hooks.json` is anchored on the inner broadcast tools and `.github/workflows/ci.yml`'s negative-match list asserts the `*_orchestrated` wrappers don't match. So the branch isn't about avoiding a permission prompt; it's about avoiding elicitation ceremony for a read-only path.) ENG-212 (split lookup into its own orchestrated MCP tool — one that doesn't elicit) is in Backlog and would block ENG-130 if treated as a dependency. The 3-line skill-side branch is the cheapest fix and collapses cleanly when ENG-212 lands.
...
- **manage-domain** — `lookup` calls `mcp__manifest-lease__lease_by_custom_domain` directly to skip the orchestrated wrapper's state-change-shaped elicitation ceremony (fee estimate, intent confirm, on-chain verify) — unnecessary overhead for a pure chain query. `set` and `clear` route through `mcp__manifest-agent__manage_domain_orchestrated`, which handles fee estimation, textual confirmation (via elicitation), broadcast, and on-chain verification. See DECISION 5; collapses to the orchestrated form when ENG-212 lands.
