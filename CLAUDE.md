...
- **DECISION 1 — `author-manifest` stays plugin-side.** `manifest-mcp-agent` ships no `build_manifest_preview_orchestrated` tool (would have been an upstream ENG-204-tier ticket). The standalone draft-creation flow remains in the plugin over the surviving `save-manifest-draft.cjs` + `merge-env.cjs` helpers. The orchestrated `deploy_app_orchestrated` flow handles inline authoring inside its own elicitation when invoked without a saved spec — but `deploy_app_orchestrated`'s `validateSpec()` requires a complete `DeploySpec`, so the rewired `deploy-app` skill takes one input (a file path) and points non-file input at `/manifest-agent:author-manifest`.
...
- **DECISION 4 — journal mechanism is option-a (skill-side, single-entry-per-orchestrated-call).** The skill prose pipes a record to `journal-write.cjs` with ONE `tool_calls[]` entry per orchestrated invocation. `args_redacted` is produced by `_journal.cjs#redactArgs`'s per-tool reducer (added in ENG-130 for the four new tools). `result_summary` is mined from the orchestrated tool's structured return value. The journal does NOT enumerate inner broadcasts the wrapper dispatches — see "Operation journal" below for the full fidelity-trade-off discussion. The alternative (wrapper-side journal writes) was rejected to keep the plugin's secret-key denylist + record schema out of the upstream package.
...
- **restart-app** — Restart a running app via `restart_app` without closing the lease. Per the `scripts/session-start.sh` runtime policy, `restart_app` is a provider HTTPS call, NOT a Cosmos broadcast — no gas, no fee estimate, no `cosmos_estimate_fee` step. The skill inlines its own textual confirm. PreToolUse still gates the tool. Pre-call: refuse if `chainState.state !== 1` (inline enum decode; the chain proto's `LEASE_STATE_ACTIVE` is stable per ENG-158). Post-call: re-query `app_status` once and inline-decode the new state; tag the journal `recovery_actions` with `["restart-post-verify-not-active"]` on regression.
