...
- **manage-domain** — `lookup` calls `mcp__manifest-lease__lease_by_custom_domain` directly; `set` and `clear` route through `mcp__manifest-agent__manage_domain_orchestrated` (which handles fee estimation, textual confirmation via elicitation, broadcast, and on-chain verification). The lookup branch is process drift, not design intent — the orchestrated wrapper's lookup sub-flow has the same elicitation-capability carve-out and dispatches to the same chain query under the hood, so neither form is substantively better. The pre-rewire skill used the direct call and the rewire kept it. See DECISION 5; collapses to a future `lookup_custom_domain_orchestrated` form when ENG-212 lands.
...
**5. Reference by structural role, not by index.** Hardcoded indices into living lists ("branch 5," "the third bullet," "field 4") drift silently as the list grows. Describe items by semantic role ("the unknown-tool fallback," "the chain-state branch") instead. **Why:** PR #9 R5 caught the `_journal.cjs` `redactArgs` non-object routing comment referencing "branch 5 in the header docstring" when the docstring had been updated to enumerate seven branches (fixed in commit `dec2cf1`); same pattern caught in DECISION-4 wording and runtime-policy bullet positions. **How to apply:** when writing a cross-reference to a list element, prefer the semantic-role form ("the unknown-tool fallback branch") over the index form ("branch 5") and prefer a commit hash over a line number for change-history pointers.

**6. Sweep discipline (paired fixer + reviewer).** When landing a drift-class fix, grep the entire repo for the same prose pattern — not just the touched file. When reviewing a drift-class fix, independently run the same grep — don't trust the commit's stated coverage. **Why:** PR #9 R6 fixed a wrong rationale at two of three sites; QA caught the third site (the `manage-domain` Skills-section bullet) where the same prose pattern lived (propagation fix in commit `5e7411a`). Either discipline alone misses what the pair catches. **How to apply:** see [`CONTRIBUTING.md`](CONTRIBUTING.md) "Sweep discipline".
...
6. **Sweep your own additions against every principle in this section before pushing.** Discipline-codifying PRs are the canonical first violators of the disciplines they codify: PR #11 (this very section's codifying PR) self-violated the *Reference-by-structural-role* principle in R1 (worked-example pointers in the prose teaching "don't cite indices" were themselves file-line citations rather than commit-hash + semantic-role pairs; fixed in commit `5fd733f`), the *Sweep-discipline* principle in R3 (a R2 mechanism correction in `docs/testing.md` wasn't swept to the matching summary in CLAUDE.md; fixed in commit `05ea912`), and the *Sweep-discipline* principle again in R4 (a meta-corollary commit said "names survive renumbering" without sweeping the bare-number cross-references in `docs/testing.md` + `CONTRIBUTING.md` + `ci/policy-completeness.cjs`; fixed in commit `c0fdfcd`). All three caught by Copilot review, not by the author. Before pushing a hindsight-codifying commit, re-read your own additions against each landed principle by name (not just by number — names are durable across renumbering), and `grep -rn --exclude-dir=.git --exclude-dir=node_modules '<prose-pattern>' .` (options before pattern/path for BSD/macOS grep portability; matches the guidance in `CONTRIBUTING.md` "Sweep discipline") for any pattern your additions illustrate. The first reviewer of a hindsight is the hindsight itself — make it count.
