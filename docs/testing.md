...
- **`set -e`/`pipefail` lying-green.** Blocks ran via `bash -c` without `-e -o pipefail`, so a multi-command block where an earlier command failed and the last succeeded exited 0 — docs-ci lied green. Caught by Copilot R4. Fix added the shell flags + two regression tests (`false; echo done` and `nonexistent | cat`) each independently red-green'd. Commit `e5008ed`.
