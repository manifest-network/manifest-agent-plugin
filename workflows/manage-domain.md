...
- If the lease exists, surface `fqdn` and `lease.leaseUuid`. Suggest
  `{{invoke:troubleshoot-deployment}} <leaseUuid>` for its inventory.
  This result does not contain tenant, provider, or service details.
- If `lease` is null, tell the user the FQDN is not
  currently claimed and that `{{invoke:manage-domain}}` → "set"
...
```
{{tool:agent/manage_domain_orchestrated}}({
  action: ACTION,
...
`{{tool:agent/lookup_custom_domain_orchestrated}}`
