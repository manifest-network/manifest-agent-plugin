...
Call `{{tool:fred/app_status}}({ lease_uuid: LEASE_UUID })`.
...
> `{{invoke:deploy-app}}` to redeploy if the lease has closed,
> or `{{invoke:troubleshoot-deployment}} <LEASE_UUID>` to see
> what state the lease is actually in.

## Step 3 — Mainnet warning (if applicable)

If `activeChain === "mainnet"`, ask via `{{ask}}`:
...
Call `{{tool:fred/restart_app}}({ lease_uuid: LEASE_UUID })`.
{{direct_permission}}
...
Re-call `{{tool:fred/app_status}}({ lease_uuid: LEASE_UUID })`
...
- If the provider reports `failed`, surface `fredStatus.reason` and
  ...
  `{{invoke:troubleshoot-deployment}} <LEASE_UUID>`. An unknown
