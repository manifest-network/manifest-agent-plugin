...
description: >
  ...
  by {{invoke:author-manifest}}. Without an argument, points the
  user at {{invoke:author-manifest}} — the orchestrated tool does
...
`{{tool:agent/deploy_app_orchestrated}}` (in the `manifest-agent`
...
- If `{{arguments}}` is a readable file path, `{{read_tool}}` it and parse the
...
- Otherwise (empty, image-shaped string, unreadable path) tell the
  ...
  `{{invoke:author-manifest}}` to build one interactively, then
  re-run `{{invoke:deploy-app}} <path>` with the saved file."
...
and ask the user to select a SKU through `{{invoke:author-manifest}}`;
...
Set `SPEC_PATH` to its shell-quoted path in this {{shell_tool}} call; never
...
Call `{{tool:agent/deploy_app_orchestrated}}({ spec: SPEC })`.
...
{{host}} renders these requests and returns the user's answers; do
...
- Follow-up: `{{invoke:troubleshoot-deployment}} <leaseUuid>`
...
- `DEPLOY_READINESS_UNCONFIRMED` or `details.partial === true`: remote
  ...
  existing lease with `{{invoke:troubleshoot-deployment}}`.
