...
description: >
  ...
  {{invoke:deploy-app}} run shows the app unhealthy, when an
...
Manifest. The orchestrated tool `{{tool:agent/troubleshoot_deployment_orchestrated}}`
...
`{{tool:agent/close_lease_orchestrated}}` for cleanup.
...
4. **Lookup by custom domain**: when the user picks this option, ask
   ...
   ```
   {{tool:agent/lookup_custom_domain_orchestrated}}({ fqdn: <fqdn> })
   ```
...
```
{{tool:agent/troubleshoot_deployment_orchestrated}}({ lease_uuid: LEASE_UUID })
```
...
`{{tool:fred/app_status}}`,
`{{tool:fred/app_diagnostics}}`, or
`{{tool:fred/get_logs}}` with the lease UUID
...
After printing the report, ask the user via `{{ask}}` whether to
close the lease:
...
```
{{tool:agent/close_lease_orchestrated}}({ lease_uuid: LEASE_UUID })
```
...
numeric fee estimate. {{host}} renders the elicitation request
